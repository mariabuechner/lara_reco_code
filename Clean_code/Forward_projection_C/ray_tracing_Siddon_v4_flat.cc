...
#define MAXZ_SLICE 180    // number of slices of phantom to load while computing a single projection
...
int main() { 
  ...
  Point d{0.001953125,0.001953125,0.001953125};        // voxel dimensions in [cm]
  Point b{-1.0, -1.0, -1.0};                           // intersection of planes with indices 0 in [cm]
  double r = 1.0;                                      // radius of FOV in [cm]
  double R = 3.0;                                      // radius of sourece trajectory in [cm]
  double D = 2.0 * R;                                  // detector-source distance in [cm]
  double H = 0.5;                                      // detector height in [cm]
  double P = 0.2831794495423;                          // source trajectory pitch in [cm]
  double delta_w = H / NR;
  double delta_u = H / NR;
  double delta_alpha = H / NR / D;
  double u_shift = 0.0;                                 // shift_detector[0] in [cm]
  double w_shift = 0.0;                                 // shift_detector[0] in [cm]
  double u[NC];
  linspace(-delta_u / 2 * (NC - 1) - u_shift,
           delta_u / 2 * (NC - 1) - u_shift,
           NC, u);
  double w[NR];
  linspace(-delta_w / 2 * (NR - 1) - w_shift,
           ...
           NR, w);
  double s[NP];
  double ds = H / NR / r;                                // source trajectory sampling
  arange(-8.688592371732, -1.725733245644, NP, ds, s);   // (smin, smax, number of porjections, ds, s)
  Point sp[NP];
  for (int i = 0; i < NP; ++i) {
  ...
  siddon_cone_beam_projection_flat_detector("Phasephantom4.h5",
                              sp, s, u, w, r, R, D, H, N, d, b, Df,
                              /*dpc=*/true);
  write_to_file("siddon_projections_1024_phase_32_276_446_flat.h5", Df);
  return 0;
}
