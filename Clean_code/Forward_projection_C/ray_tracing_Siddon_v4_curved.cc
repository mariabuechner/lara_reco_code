...
int main() {
  ...
  double P = 0.3003571635;				             // source trajectory pitch in [cm]
  double delta_w = H / NR;
  double delta_alpha = H / NR / D;
  double alpha_shift = 0.0;                          // shift_detector[0] in [cm]
  double w_shift = 0.0;							     // shift_detector[1] in [cm]
  double alpha[NC];
  ...
  linspace(-delta_w / 2 * (NR - 1) - w_shift,
           ...
           NR, w);
  double s[NP];
  double ds = 0.5 / 32 / 1.0;                           // source trajectory sampling
  arange(-8.37135414150, -1.408495015419, NP, ds, s); // (smin, smax, number of porjections, ds, s)
  Point sp[NP];
  for (int i = 0; i < NP; ++i) {
  ...
  siddon_cone_beam_projection("Phasephantom4.h5",
                              sp, s, alpha, w, r, R, D, H, N, d, b, Df,
                             /*dpc=*/true);
							                        //phantom4.h5 - file name of the phantom data
							                        // dpc -> false - absorption, true - phase
  write_to_file("siddon_projections_1024_phase_32_276_446.h5" Df);  
  return 0;
}
