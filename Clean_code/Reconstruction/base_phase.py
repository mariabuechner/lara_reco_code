...
def getArgs():
    ...
    parser.add_argument('-Nr', '--Nr', dest='Nr', type=np.int16, default=32,
                        help = 'Number of detector rows')   
    parser.add_argument('-Nc', '--Nc', dest='Nc', type=np.int16, default=276,
                        help = 'Number of detector columns')
    parser.add_argument('-dew', '--dew', dest='dew', 
                        type=np.float64, default=0.5/32,
    ...
    parser.add_argument('-ds', '--ds', dest='ds', 
                        type=np.float64, default=0.5/32/1.0,
    ...
    parser.add_argument("-projf", "--projFileName", type=str, 
                        default = 'siddon_projections_1024_phase_32_276_446.h5',
...
def init_scan_geometry(args):
    '''
    Creates dictionary structure of input arguments which is used in 
    functions inside forward_projectio.py.
    Input: args - argparse arguments
    Output: settings - dictionary structure of argparse arguments3
    '''
    settings = {}
...
def nextpow2(i):
    '''
    Finds next power of 2 larger than i (used in filtering)'
    Input: i - intger
    Outpu: n - next power of 2 larger than i.
    '''
    n = 1
    while n < i: n *= 2
    return n
...
def k_line_point_w(D, P, R, psi, alpha):
    ...
    Computes omega coordinate of a point on the k-line with angular
    coordinate alpha [Noo et al. equation 26].
    ...
    Output: omega coordinate of a sample on the k-line for a given 
            alpha coordinate
    '''
    const = D*P / (2*pi*R)
    if np.abs(psi) < 1e-5:
...
def k_line_point_w_flat_detector(D, P, R, psi, u):
    '''
    Computes omega coordinate of a point on the k-line with detector coordinate u
    [Noo et al. equation 72] 
    Input: D - radius of detector
           P - pitch of spiral source trajectory 
           R - radius of spiral source trajectory
           psi - angle defining k-plane
           u - flat detector coordinate in direction perpendicular to rotation
               axis 
    Output: omega coordinate of a sample on the k-line for a given u coordinate
    '''
    const = D*P / (2*pi*R)
    if np.abs(psi) < 1e-5:
        # psi = 0, psi/tan(psi) = 1
...
def number_k_lines(alpha_m, D, P, R, delta_w):
    ...
    '''
    N = ceil(D*P*(pi+2*alpha_m) / (pi*R*cos(alpha_m)*delta_w) * \
...
def zeta_scaling(Df, s, alpha, delta_alpha, delta_w, R, D, P, Nr, Nc, \
                                                        shift_detector):
    '''
    Curved detector function used in phase-contrast reconstruction.
    ...
    for p in range(Np):
        ...
        for i in range(Nr):
            for j in range(Nc): 
                ...
                rgl1gl2 = np.dot(rgl1, rgl2)
                zeta = np.dot(derys[:,p],rgl1) - \
                       np.dot(derys[:,p],rgl2) * rgl1gl2
                zeta = zeta/(1 - rgl1gl2**2)
                g1[p,i,j] = zeta * Df[p,i,j]

    ''' FileName = 'zeta_scaling.h5' 
    f = h5py.File(FileName, 'w') 
    f.create_dataset('g1', data=g1, compression='gzip', \
                            compression_opts=9)
    f.close() '''
    return g1
...
def Hilbert_filter(Ns, shift_detector, sample_step):
    ...
    [MSc thesis equation 2.55] 
    Input: Ns - number of samples
    Output: h1 - Hilbert filter samples
    '''
    if np.mod(Ns,2)==0:
...
def Hanning_sin(Ns, shift_detector, sample_step):
    ...
    Output: h1- Hanning window samples
    ''' 
    shift = - shift_detector[0]/sample_step + 0.5
    h1 = np.zeros((1,Ns), dtype = TYPE)
    for i in np.arange(Ns):
...
def filter_klines_slow(klines, Np, Nk, Nc, shift_detector, sample_step):
    '''
    Returns filtered k-lines. Fltering done for each projection in a for loop 
    because zero padding all projection causes memory problems.
    Input: klines - k-lines for one source point position
           Np - number of projections
           Nk - number of k-lines
           Nc - number of detector columns
...
def proj2klines(g2, psi, alpha, w, delta_w, D, P, R, Np, Nc, Nr, Nk):
    ...
    Input: g2 - projections (s, w, alpha)
           ...
           Nr - number of detector rows
           Nk - number of k-lines
    Output: k-lines samples (s, psi, alpha)              
    '''
...
def klines2proj(g4, wk, w, delta_w, Np, Nc, Nr, Nk):
    ...
    Input: g4 - filtered klines (s, psi, alpha/u)
           ...
           Nr - number of detector rows
           Nk - number of k-lines
    Output: k-line samples re-mapped onto the detector grid
    '''
...
def PIpoint(x3, r, gamma, R, h, sb):
    '''
    Solving nonlinear function f=0 gives solution for sb parameter
    [Wunderlich page 22]
    ...
    for row in range(Nx):
        ...
        for col in range(Ny):
            if (xc[row]**2 + yc[col]**2) < r**2:
               ...
               # indices of the nearest neighbour coordinates
...
def absorption_reconstruction(Df, x3, args):
    ...
    Nk = 2*np.round(Nkmin).astype(np.int)
...
def phase_reconstruction(Df, x3, args):
    ...
    # compensation for no derivation
    Nc = Nc+1
    Np = Np+1
    shift_detector = shift_detector - np.array([0.5*delta_alpha, 0])

    g2 = cos_lambda_scaling(g1, w, D, Np, Nr, Nc)
    g3, wk = proj2klines(g2, psi, alpha, w, delta_w, D, P, R, Np, Nc, Nr, Nk)
    g4 = filter_klines_slow(g3, Np, Nk, Nc, shift_detector, delta_alpha)          
    g5prime = klines2proj(g4, wk, w, delta_w, Np, Nc, Nr, Nk)
    g5 = g5prime[:,:,:-1]
    # compensation no needed     
    shift_detector = shift_detector + np.array([0.5*delta_alpha, 0]) 
    Nc = Nc-1   
    Np = Np-1
    g6 = cos_alpha_scaling(g5, alpha, Np, Nr, Nc)
...
def phase_reconstruction_flat_detector(Df, x3, args):
    ...
    Nc = Nc+1
    Np = Np+1
    shift_detector = shift_detector - np.array([0.5*du, 0.5*dw])

    g2 = cos_lambda_scaling_flat_detector(g1, w, u, D, Np, Nr, Nc)
    g3, wk = proj2klines_flat_detector(g2, psi, u, w, dw, D, P, 
                                       R, Np, Nc, Nr, Nk)
    g4 = filter_klines_slow(g3, Np, Nk, Nc, shift_detector, du)          
    g5prime = klines2proj(g4, wk, w, dw, Np, Nc, Nr, Nk)
    g5 = g5prime[:,:,:-1]
    # compensation no needed   
    shift_detector = shift_detector + np.array([0.5*du, 0.5*dw])
    PI = PIinterval_range(xc, yc, x3, Nx, Ny, r, R, h, PIUseFile, PIFileName)
...
def forward_projections(x3, args, dpc):
    ...
    if projUseFile: 
        ...
        #print Np, smin, smax, ds
        #print slice_range(0,1024,H,D,R,r)
        #sys.exit()
...
if __name__ == '__main__':
    ...
    if args.imaging == 'absorption':
        ...
        if args.detector == 'curved': 
            ...
            # reconstructed slice z = x3
            print 'Reconstruction flat detector absorbtion'
        ...
        if args.detector == 'curved': 
            ...
            print 'Reconstruction flat detector phase'
            rec_slice = phase_reconstruction_flat_detector(Df, x3, args)
        else:
            sys.exit('''\nError: Unknown detector type.''')
        ...
        plt.colorbar()
        plt.show()
        recFileName = 'rec_phase_' + args.projFileName 
        recf = h5py.File(recFileName, 'w') 
        recf.create_dataset('rec_slice', data=rec_slice, compression='gzip', \
                                compression_opts=9)
        recf.close() 
    ...
    phantomFileNameSlice = args.imaging + 'phantom_slice.h5'
