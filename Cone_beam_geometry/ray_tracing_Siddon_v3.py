'''
Third version of ray tracing algorithm. To save memory the gradient is locally
computed when its needed, so there is no need to store 3 * the memory that uses
phantom. 
Computation of gradient locally slows down the execution. 
'''
import numpy as np
