...
int main() {
  ...
  Point d{0.03125, 0.03125, 0.03125};
  Point b{-1.0, -1.0, -1.0};
  double r = 1.0;
  double R = 3.0;
  double D = 2.0 * R;
  ...
  double s[NP];
  double ds = 0.5 / 16;
  arange(-8.5456795244381141, -2.9421680361667706, NP, ds, s);
  Point sp[NP];
  for (int i = 0; i < NP; ++i) {
  ...
  siddon_cone_beam_projection("phantom_64.h5",
                              sp, s, alpha, w, r, R, D, H, N, d, b, Df,
                              /*dpc=*/false);
  write_to_file("df_64.h5", Df);
  return 0;
}
