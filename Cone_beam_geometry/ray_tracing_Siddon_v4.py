'''
Forth version of ray tracing algorithm. The gradient is precomputed for only 
that part of phantom through which the cone beam x-rays are passing for a 
...
def siddon_xray_transform(obj, dp, sp, r, D, Nx, Ny, Nz, dx, dy, dz, 
    ...
    if vox_coor[0]>(Nx-1) or vox_coor[1]>(Ny-1) or vox_coor[2]>(Nz-1) or \
        vox_coor[0]<0 or vox_coor[1]<0 or vox_coor[2]<pozk:
            return prj
...
def siddon_cone_beam_projection(objFileName, sp, s, alpha, w, Nr, Nc, r, R, D, 
                            H, Nx, Ny, Nz, dx, dy, dz, bx, by, bz, dpc=False):
    '''
