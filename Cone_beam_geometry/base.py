...
def getArgs():
    ...
    geometry with curved/flat detector. The 3D Shepp-Logan phantom  
    ...
    parser.add_argument('-Nr', '--Nr', dest='Nr', type=np.int16, default=2*16,
                        help = 'Number of detector rows')   
    parser.add_argument('-Nc', '--Nc', dest='Nc', type=np.int16, default=2*138,
                        help = 'Number of detector columns')
    parser.add_argument('-dew', '--dew', dest='dew', 
                        type=np.float64, default=0.5/16/2,
    ...
    parser.add_argument('-ds', '--ds', dest='ds', 
                        type=np.float64, default=0.5/16/6.0,
    ...
    parser.add_argument("-phaf", "--phantomFileName", type=str, 
                        default = 'big_phantoms/phantom1.h5',
...
def number_k_lines_flat_detector(alpha_m, D, P, R, delta_w):
    ...
    Output: number of k-lines (even number) 
    '''
    N =ceil(D*P*(pi+2*alpha_m) / (2*pi*R*delta_w) * \
...
def phase_reconstruction_flat_detector(Df, x3, args):
    ...
    g5 = klines2proj(g4, wk, w, dw, Np, Nc, Nr, Nk)
    PI = PIinterval_range(xc, yc, x3, Nx, Ny, r, R, h, PIUseFile, PIFileName)
    rec_slice = backprojection(g5, ys, s, xc, yc, x3, Nx, Ny, ds, r, R, D, 
                                shift_detector, dw, du, u, w, Nr, PI) 
    #------------------------------------------------------------------------# 
    return rec_slice  
...
if __name__ == '__main__':
    ...
    phantomFileNameSlice = 'big_phantoms/'+ args.imaging + 'phantom_slice.h5'
