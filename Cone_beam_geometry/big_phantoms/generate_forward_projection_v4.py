...
def getArgs():
    ...
    parser.add_argument('-Nr', '--Nr', dest='Nr', type=np.int16, default=16*2,
                        help = 'Number of detector rows')   
    parser.add_argument('-Nc', '--Nc', dest='Nc', type=np.int16, default=138*2,
...
if __name__ == '__main__':
    ...
    Pmax = maxPitch(D, Nr/2, R, r, 2*delta_w) # maximal pitch
    P =  args.Pfactor * Pmax
    h = P/(2*pi)    
    ...
    projf.create_dataset('Df', data=temp, compression='gzip', \
                               compression_opts=9) 
    projf.attrs['D'] = D
    projf.attrs['H'] = H
    projf.attrs['Nr'] = Nr 
    ...
    print 'Stopped to set temp_190.h5 to some other name!'    
    sys.exit()
    ...
    '''
    plt.figure()
    res = plt.imshow(temp[:,0,:], cmap=plt.cm.gray, \
                     interpolation='none')
    ...
    plt.figure()
    res = plt.imshow(b[-1,:,:]-c[0,:,:], cmap=plt.cm.gray, \
                     interpolation='none')
